...
def tile_alt_imshow(img_arrays, labels=None, titles=None, label_choice=1, width=30, height=30, save_it=False):
    scaled_img_arrays = rescale_img(img_arrays)
    img_n, img_h, img_w, _ = img_arrays.shape

    n_slots = int(math.ceil(np.sqrt(img_n)))

    fig, axes = plt.subplots(n_slots, n_slots, figsize=(width, height))
    ...
    for ax, i in zip(axes.flatten(), range(img_n)):
        ...
        ax.axis('off')
    if save_it:
        plt.savefig('PTX_samples', dpi=300, format='png')
    else:
        plt.show()
