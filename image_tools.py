...
def tile_alt_imshow(img_arrays, heat_maps=None, labels=None, titles=None, label_choice=1,
    ...
    for ax, i in zip(axes.flatten(), range(img_n)):
        ...
        cxr = ax.imshow(img, cmap='gray')

        if heat_maps is not None:
            resized_map = transform.resize(heat_maps[i], (300, 300), mode='reflect', anti_aliasing=True)
...
def single_superimposed_images(image, small_map, color_scheme='seismic', alpha=0.4, save=False,
                               height=10, width=10, format='eps', dpi=600):

    f, ax = plt.subplots(1, 1, figsize=(height, width))

    cxr = ax.imshow(image, cmap='gray')
    resized_map = transform.resize(small_map, (300, 300), mode='reflect', anti_aliasing=True)
    plot_heat_map = ax.imshow(resized_map, cmap=color_scheme, alpha=alpha, vmin=0.0, vmax=1.0)
...
def show_histogram(img_array, title='Histogram for test set', save_name=None):
    imgs_flat = img_array.flatten()
    n, bins, patches = plt.hist(imgs_flat, 10)
    plt.xlabel('Pixel value')
    plt.ylabel('Counts')
    plt.title(title)
    print('mean: {}, std: {}'.format(np.mean(imgs_flat), np.std(imgs_flat)))
    if save_name is not None:
        plt.savefig(save_name, format='png', dpi=400)
    plt.show()