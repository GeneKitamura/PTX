...
def load_pandas(path_to_hdf='./data.h5', combined=True, test_box_path=None):
    ...
    with pd.HDFStore(path_to_hdf, mode='r') as store:
        ...
        other_training_hold = store.get('other_train_hold')
        other_testing_hold = store.get('other_test_hold')

    if combined: # concat sort is for sorting the column titles in alphabetical order
...
def csv_pandas(data_csv='./chest_8/Data_Entry_2017.csv', box_csv='./chest_8/BBox_List_2017.csv', finding='Pneumothorax', img_size=(1024, 1024), binary_classification=True):
    ...
    if binary_classification:
        ...
        # TODO: All patches should be label 0 for non_path cases
        non_path_cases['nu'] = 1
